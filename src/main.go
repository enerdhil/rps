	...
	if desk, ok := a.(desktop.App); ok {
		m := fyne.NewMenu("MyApp",
			...
			fyne.NewMenuItem("Update", func() {
				fyneselfupdate.NewUpgradeConfirmCallbackWithTimeout(w, time.Duration.Seconds(5))
				fyneselfupdate.NewProgressCallback(w)
				fyneselfupdate.NewRestartConfirmCallbackWithTimeout(w, time.Duration.Seconds(5))
			}))
